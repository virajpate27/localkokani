      ...
      {value.length === 0 ? (
        ...
        <div className="space-y-3">
          {value.map((room, index) => (
            <div
              key={index}
              className="grid grid-cols-1 sm:grid-cols-[1fr_140px_100px_auto] gap-3 items-center border dark:border-gray-800 rounded-xl p-3"
            >
              <input
                type="text"
                value={room.name}
                ...
                className="px-3 py-2 rounded-lg border dark:border-gray-800 focus:border-secondary text-sm outline-none"
              />
              <input
                type="number"
                value={room.price}
