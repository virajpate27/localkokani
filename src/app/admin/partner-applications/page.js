...
export default function AdminPartnerApplicationsPage() {
  ...
  useEffect(() => {
    getAllPartnerApplicationsAdmin().then(setApplications).finally(() => setIsLoading(false));
  }, []);
        ...
        {["all", "pending", "approved", "rejected"].map((s) => (
          <button
            ...
            className={`text-sm font-medium px-4 py-2 rounded-lg capitalize transition-colors ${
              statusFilter === s ? "bg-primary text-white" : "bg-white text-gray-500 hover:bg-gray-100 border border-gray-200"
            }`}
          >
            {s}
          </button>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((app) => (
                <tr key={app.id} className="hover:bg-gray-50">
                  <td className="px-5 py-3.5 font-medium text-primary">{app.property?.name}</td>
                  <td className="px-5 py-3.5 text-gray-600">{app.owner?.fullName}</td>
                  <td className="px-5 py-3.5 text-gray-600 capitalize">{app.property?.type}</td>
                  <td className="px-5 py-3.5 text-gray-600 capitalize">{app.plan}</td>
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <Link href={`/admin/partner-applications/${app.id}`} className="text-secondary font-medium hover:underline">
                      Review
                    </Link>
                  </td>
                </tr>
