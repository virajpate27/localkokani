...
import {
  ...
  cancelPromotionRequest, hasActiveOrScheduledPromotion, getExtendableRequestsByOwner, addDays,
  DURATIONS,
...
function PromoteContent() {
  ...
  useEffect(() => {
    async function load() {
      const [hotels, restaurants, pricingData, requestsData, extendableData] = await Promise.all([
  ...
  const handleSubmit = async () => {
    ...
    try {
      const alreadyHasOne = await hasActiveOrScheduledPromotion(selectedListing.id, promotionType);
      ...
      toast.success(extendingFrom ? "Extension request submitted!" : "Request submitted! We'll confirm once payment is received.");
        ...
        <div className="card p-5 bg-secondary/5 border border-secondary/20">
          ...
          <div className="space-y-2">
            {extendableRequests.map((req) => (
              <div key={req.id} className="flex items-center justify-between gap-3 bg-white rounded-lg p-3">
                <div>
                  <p className="text-sm font-medium text-primary">
                    {req.entityName} — <span className="capitalize">{req.promotionType}</span>
                  </p>
                  <p className="text-gray-400 text-xs">
                    {req.status === "active" ? "Live until" : "Starts"} {req.status === "active" ? req.endDate : req.startDate}
                  </p>
                </div>
                <button
      ...
      <div className="card p-6 space-y-5">
        ...
        {extendingFrom && (
          <div className="bg-secondary/5 border border-secondary/20 rounded-xl p-3 text-sm text-gray-600">
            Extending <strong>{extendingFrom.entityName}</strong>'s {extendingFrom.promotionType} promotion.
            Your new period will start right after the current one ends — no gap in visibility.
          </div>
        )}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              type="date"
              value={startDate}
              min={extendingFrom ? startDate : new Date().toISOString().split("T")[0]} // lock the min to the computed continuation date while extending
        ...
        <button onClick={handleSubmit} disabled={isSubmitting} className="btn-primary w-full disabled:opacity-60">
          {isSubmitting ? "Submitting..." : extendingFrom ? "Submit Extension" : "Submit Request"}
        </button>
